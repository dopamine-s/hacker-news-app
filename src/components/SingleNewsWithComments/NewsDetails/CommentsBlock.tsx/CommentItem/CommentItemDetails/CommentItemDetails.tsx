...
import classes from './CommentItemDetails.module.scss';
import he from 'he';
...
const CommentsItemDetails: FC<CommentsItemDetailsProps> = ({
  by,
  kids = [],
  text,
  time,
    ...
    <>
      <Card className={classes['single-comment']}>
        ...
        {decodedText ? (
          ...
          <p>{deleted ? '*Deleted' : '[dead]'}</p>
        )}
        {kids?.length > 0 && (
          <button onClick={changeShowNestedCommentsHandler}>
            {showKidsComments ? '—' : kids?.length}{' '}
            {!showKidsComments && 'More'}
          </button>
        )}
